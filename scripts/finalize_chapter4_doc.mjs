...
if (!packageDir) {
  console.error('Usage: node scripts/finalize_chapter4_doc.mjs <extracted-docx-package-dir>');
  process.exit(1);
}
...
function replaceAllChecked(input, from, to, label) {
  if (!input.includes(from)) {
    throw new Error(`Missing expected text: ${label}`);
  }
...
function replaceNth(input, from, to, occurrence, label) {
  ...
  for (let i = 0; i < occurrence; i += 1) {
    ...
    if (index === -1) {
      throw new Error(`Missing expected occurrence ${occurrence}: ${label}`);
    }
...
for (const [targetName, sourceName] of Object.entries(mediaMap)) {
  ...
  if (!fs.existsSync(source)) {
    throw new Error(`Missing final diagram: ${source}`);
  }
...
for (const targetName of Object.keys(mediaMap)) {
  const svgName = targetName.replace(/\.png$/i, '.svg');
  relsXml = relsXml.split(`Target="media/${svgName}"`).join(`Target="media/${targetName}"`);
}
...
for (const targetName of Object.keys(mediaMap)) {
  const svgName = targetName.replace(/\.png$/i, '.svg');
  documentXml = documentXml.split(svgName).join(targetName);
}
...
documentXml = replaceAllChecked(
  ...
  '2. One district can own many complaints, while one complaint belongs to one district.',
  'ERD relationship 2',
);
documentXml = replaceAllChecked(
  documentXml,
  '3. One complaint can have many comments and many reactions.',
  '3. One user can own many registered devices.',
  'ERD relationship 3',
);
documentXml = replaceAllChecked(
  documentXml,
  '4. One user can own many device registrations and many notification events.',
  '4. One user can receive many notification events.',
  'ERD relationship 4',
);
documentXml = replaceAllChecked(
  documentXml,
  '6. Queue jobs store asynchronous work such as notification delivery and reference payload data needed by workers.',
  '6. Notification events can create queue jobs for asynchronous delivery.',
  'ERD relationship 6',
);

documentXml = replaceNth(
  documentXml,
  'Figure 3. Design of user registration and login',
  'Figure 9. Authentication interface snapshot',
  2,
  'authentication snapshot caption',
);
documentXml = replaceNth(
  documentXml,
  'Figure 5. Design of complaint reporting and district assignment',
  'Figure 10. Complaint reporting interface snapshot',
  2,
  'complaint snapshot caption',
);
documentXml = replaceNth(
  documentXml,
  'Figure 7. Design of notification and update tracking',
  'Figure 11. Notification interface snapshot',
  2,
  'notification snapshot caption',
);

const relIdByTarget = new Map();
for (const match of relsXml.matchAll(/<Relationship\b[^>]*Id="([^"]+)"[^>]*Target="media\/([^"]+)"/g)) {
  relIdByTarget.set(match[2], match[1]);
}

for (const [targetName] of Object.entries(mediaMap)) {
  const relId = relIdByTarget.get(targetName);
  if (!relId) {
    throw new Error(`No relationship found for ${targetName}`);
  }
  const imagePath = path.join(mediaDir, targetName);
  ...
  const inlinePattern = /<wp:inline[\s\S]*?<\/wp:inline>/g;
  let replaced = 0;
  documentXml = documentXml.replace(inlinePattern, (block) => {
  ...
  if (replaced === 0) {
    throw new Error(`No drawing block found for ${targetName} (${relId})`);
  }
}

fs.writeFileSync(documentPath, documentXml);

console.log(`Finalized ${Object.keys(mediaMap).length} media files and updated Chapter 4 text.`);
